...
```{r}
fish <- readr::read_csv("lab4_data/Gaeta_etal_CLC_data.csv")
```
```{r}
View(fish)
```
...
```{r}
mammals <- readr::read_csv("lab4_data/mammal_lifehistories_v2.csv")
```

```{r}
mammals <- replace(mammals, mammals==-999.0, NA)
```
...
There are NA's represented by the numeric value -999.0, which I replaced with "NA" in the dataframe.
```{r}
...
```{r}
select(new_mammals, -order, -family)
```
