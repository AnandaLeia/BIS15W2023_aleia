...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv("lab10_data/mammal_lifehistories_v2.csv", na="-999") %>% clean_names()
life_history
```
...
```{r}
elephants <- read_csv("lab10_data/elephantsMF.csv") %>% clean_names()
```
```{r}
str(elephants)
elephants
...
elephants %>% 
  ...
  geom_point(na.rm=T)+
  geom_smooth(method=lm)+
  labs(title = "Elephant Age versus Height",
       x = "Age",
       y = "Height")
```
...
life_history %>% 
  ...
  geom_point(na.rm=T)+
  labs(title = "Animal Gestation by Mass",
       y = "log10 of Mass",
       x = "Length of Gestation")
```
```{r}
life_history %>% 
  filter(gestation>=20)
