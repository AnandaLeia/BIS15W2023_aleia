...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) #lapply applies the function read_csv to everything identified in files from the previous code chunk
```

We can view elements in our list with double brackets. Let's view the data for Butte county.
```{r}
spider_list[[3]] #use double brackets to select out observation from the list
```
## Practice
...
table_B
```
There are different kinds of joins for different kinds of desired outcome
...
inner_exampleDF
#inner joins datasets by creating a new dataset that only includes the observations that appear in both original datasets under a specified variable
```
`left_join`
...
left_exampleDF
#left joins datasets by taking the observations of the first dataset, and adding on any column from the second dataset for those observations; it doesn't add observations from the second dataset though
```
...
right_exampleDF
#right joins datasets by taking observations from the second dataset and adding on any column from the first dataset for those observations; it doesn't add observations from the first set though
```
...
full_exampleDF
#full joins by combining all observations and variables from both datasets, and filling in NA's for any info not included
```
...
anti_exampleDF
anti_example2DF <- anti_join(table_B, table_A, by="customer_ID")
anti_example2DF
```
...
mdy_hms(dateformat4)
ymd_hms(dateformat5)
#identify the format that the date is entered in your dataframe, then pick the correct function for the given date format for R to convert it into the standard date format.
```
