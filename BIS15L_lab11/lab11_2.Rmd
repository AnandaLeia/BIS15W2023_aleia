...
```{r message=FALSE, warning=FALSE}
homerange <- read_csv("lab11_data/Tamburelloetal_HomeRangeDatabase.csv")
```
...
```{r}
my_palette <- paletteer_d("yarrr::cars")
```
...
```{r}
ecosphere <- read_csv("lab11_data/ecs21351-sup-0003-SupplementS1.csv", skip=2, col_names = T) %>% 
  clean_names() %>% 
