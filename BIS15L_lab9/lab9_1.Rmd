...
```{r}
ggplot(data = iris,
       mapping = aes(Species, Petal.Length))
```
...
```{r}
ggplot(data = iris,
       mapping = aes(Species, Petal.Length)) + geom_boxplot()
```
...
```{r}
ggplot(data = iris,
       mapping = aes(Sepal.Length, Sepal.Width)) + geom_jitter()
```

```{r}
ggplot(data = iris,
       mapping = aes(Sepal.Length, Sepal.Width)) + geom_point()
```
...
```{r}
homerange <- read_csv(here("lab9_data", "Tamburelloetal_HomeRangeDatabase.csv"))
```
...
```{r}
ggplot(homerange, 
       mapping = aes(log10.mass, log10.hra)) + geom_point()
```
...
```{r}
ggplot(homerange, 
       mapping = aes(log10.mass, log10.hra)) + geom_jitter()
```

To add a regression (best of fit) line, we just add another layer.
```{r}
ggplot(homerange, 
       mapping = aes(log10.mass, log10.hra)) + geom_jitter() + geom_smooth(method=lm, se=F)
```
...
```{r}
ggplot(homerange, 
       mapping = aes(log10.mass, log10.preymass)) + geom_jitter() + geom_smooth(method=lm, se=T)
```
