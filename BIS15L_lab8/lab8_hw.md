...
```
Location: /var/folders/82/b6g173vj5jz0h9wkj96v6mr40000gn/T//RtmpiHXCcD/downloaded_packages
...
```r
sydneybeaches <- read_csv("lab8_data/sydneybeaches.csv")
```
...
```
## here() starts at /Users/anandaleia/Desktop/BIS15W2023_aleia/BIS15L_lab8
```

The quotes show the folder structure from the root directory.

```r
sydneybeaches <-read_csv(here("lab8_data", "sydneybeaches.csv")) %>% janitor::clean_names()
```
...
```
This data is tidy (each variable has its own column, each observation has its own row, and each value has it's own cell). It is in the long format; if anything, it could be summarized to be widder.
...
```r
sydneybeaches_year_means <- sydneybeaches_means %>% 
  pivot_wider(names_from = "year",
              values_from = "mean_enterococci")
sydneybeaches_year_means
```
...
```r
sydneybeaches_year_means[which.max(sydneybeaches_year_means$"2018"),] %>% 
  select(site, "2018")
```
