...
```{r}
heartrate2 <- readr::read_csv("lab8_data/heartrate2.csv")
heartrate2
```
...
```{r}
tb_data <- read_csv("lab8_data/tb_data.csv")
tb_data
```
...
```{r}
gene_exp <- read_csv("lab8_data/gene_exp.csv")
gene_exp
```
...
```{r}
edu_level <- readr::read_csv("lab8_data/education_level.csv")
edu_level
```
...
```{r}
beachbugs <- read_csv("lab8_data/sydneybeaches.csv")
beachbugs
```
...
beachbugs_wide <-
  ...
  pivot_wider(names_from = "Region",
              values_from = "Site") %>% 
  select ("Sydney City Ocean Beaches", "Date", "Enterococci (cfu/100ml)")
beachbugs_wide
```
...
beachbugs_long <-
  beachbugs_wide %>% 
  pivot_longer(cols = "Sydney City Ocean Beaches",
               names_to = "Region",
               values_to = "Beach")
beachbugs_long
```
