...
Tidy Data: Each variable has a column, each observation has a row, each value has a cell.
...
```{r}
heartrate <- read_csv("lab8_data/heartrate.csv")
heartrate
```
Problems with above data in R:
...
here()
```
Here package puts you in the main folder.
```{r}
heartrate <- read_csv(here("lab8_data2", "heartrate.csv"))
```
...
?pivot_longer
```
names_to turns the names at the head of columns into a new variable called "drug"
values_to turns all the values from across columns into values under a single new column called "heartrate"
```{r}
heartrate %>% 
...
```{r}
religion_income <- read.csv("lab8_data/relig_income.csv")
religion_income
```

2. Why are these data untidy?  
In this dataframe, the column names are data points that should be included under a separate variable.
...
```{r}
billboard <- readr::read_csv("lab8_data/billboard.csv")
billboard
```
...
```{r}
plant_data <- readr::read_csv("lab8_data/plant_data.csv")
plant_data
```

2. Why are these data not tidy?  
The columns for days are actually a variable.
...
```{r}
qpcr_untidy <- read_csv("lab8_data/qpcr_untidy.csv")
qpcr_untidy
```
...
```{r}
length_data <- readr::read_csv("lab8_data/length_data.csv")
length_data
```
