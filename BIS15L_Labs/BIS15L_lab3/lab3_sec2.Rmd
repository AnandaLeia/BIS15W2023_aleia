...
Tidyverse is a package of tools that work together. You have to install it on your computer, you should only install it once on your computer, and you only have to run it once per session.
## Load the tidyverse
...
```{r}
hot_springs <- readr::read_csv("lab3_data/hsprings_data.csv")
```
...
```{r}
fish <- readr::read_csv("lab3_data/Gaeta_etal_CLC_data.csv")
```
...
```{r}
mammals <- readr::read_csv("Lab3_data/mammal_lifehistories_v2.csv")
```
